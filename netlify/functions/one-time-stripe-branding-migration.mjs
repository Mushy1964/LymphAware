...
export default async (request) => {
  ...
  try {
    ...
    const auth = { Authorization: `Bearer ${apiKey}`, 'Stripe-Version': '2026-07-29.dahlia' };
    ...
    const updatedAccount = await stripeJson(`https://api.stripe.com/v1/accounts/${encodeURIComponent(account.id)}`, {
      method: 'POST',
    ...
    try {
      ...
      if (!logoResponse.ok) throw new Error('Unable to fetch the LymphAware ID logo.');
      ...
      if (logoFileId) brandForm.append('logo', logoFileId);
    ...
    return json({
      ...
      branding: {
        updated: brandingUpdated,
        logo_file_created: Boolean(logoFileId),
        error: brandingError
      }
    });
  } catch (error) {
